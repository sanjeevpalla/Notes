...
```mermaid
flowchart TD
    A["User asks: 'Is Interstellar showing tonight at 7pm?'"] --> B["Brain (model) invoked, no tools attached"]
    B --> C{"Does the model already<br/>know this fact?"}
    C -->|No| D["Honest refusal:<br/>'I don't have access to live showtimes'"]
    C -->|"No, but model guesses anyway"| E["Hallucination:<br/>invents a plausible-sounding showtime"]
    D --> F["Neither outcome is useful —<br/>the brain needs 'hands'"]
    E --> F
```
...
```mermaid
flowchart LR
    A["Plain Python function<br/>name + docstring + type hints"] -->|"@tool decorator"| B["Tool object"]
    B --> C["Name = function name<br/>(or override)"]
    B --> D["Description = docstring<br/>(or override)"]
    B --> E[".args = full argument schema<br/>derived from type hints"]
```
...
```mermaid
flowchart TD
    A["Tool receives a proposed argument value<br/>e.g. seat_count = 'two'"] --> B{"How is the schema defined?"}
    B -->|"Plain type hint / JSON schema"| C["No real enforcement —<br/>mismatched type silently passes through"]
    B -->|"Pydantic args_schema"| D["Genuine field + type validation"]
    D --> E{"Valid?"}
    E -->|Yes| F["Tool executes normally"]
    E -->|No| G["Validation error raised"]
```
...
```mermaid
flowchart TD
    A["Tool defined with a plain parameter<br/>named 'config' or 'runtime'"] --> B["@tool decorator runs —<br/>NO error at definition time"]
    B --> C["Tool bound to a model, agent created"]
    C --> D["Agent actually tries to INVOKE the tool"]
    D --> E["LangChain checks argument names<br/>at this point, not before"]
    E --> F["Runtime error:<br/>'reserved parameter names cannot be used'"]
```
...
```mermaid
flowchart LR
    A["User asks Claude/ChatGPT<br/>to search the web"] --> B{"Where does the<br/>tool actually run?"}
    B -->|"Built-in server-side tool"| C["Runs entirely on the<br/>PROVIDER'S infrastructure —<br/>never touches your machine"]
    B -->|"Custom @tool you define"| D["Runs on YOUR machine/server —<br/>full control, full responsibility"]
```
...
```mermaid
flowchart TD
    A["TavilySearch class<br/>(LangChain-provided)"] --> B["Same anatomy as any tool you'd write:<br/>name, description, args_schema"]
    B --> C["init() — sets up the API wrapper"]
    C --> D["run() — calls Tavily's real API<br/>using YOUR API key"]
    D --> E["Returns results back to the agent —<br/>exactly like a custom tool would"]
```
...
```mermaid
flowchart TD
    subgraph Model["What the MODEL can see"]
        A["Only the tool's declared,<br/>non-runtime arguments —<br/>its 'reflection in the mirror'"]
    end
    subgraph Tool["What the TOOL's own code can see, via runtime"]
        B["runtime.state — short-term memory"]
        C["runtime.context — immutable per-invocation config"]
        D["runtime.store — long-term memory"]
        E["runtime.stream_writer — live progress updates"]
        F["runtime.execution_info — run/thread/attempt IDs"]
    end
    Model -.->|"model never sees this side"| Tool
```
...
sequenceDiagram
    participant User
    participant Agent
    participant SaveTool as save_favorite_genre
    participant Store as runtime.store
    participant RecallTool as recall_favorite_genre

    User->>Agent: "I'm Priya, I love sci-fi movies"
    Agent->>SaveTool: call with customer_id, genre
    SaveTool->>Store: store.put((customer_id, "preferences"), "favorite_genre", {...})
    Store-->>SaveTool: saved
    SaveTool-->>Agent: "Got it, I'll remember you like sci-fi"

    Note over User,Agent: ...later, a NEW conversation...

    User->>Agent: "Recommend me something"
    Agent->>RecallTool: call with customer_id
    RecallTool->>Store: store.get((customer_id, "preferences"), "favorite_genre")
    Store-->>RecallTool: {"value": "sci-fi"}
    RecallTool-->>Agent: "Your favorite genre is sci-fi"
```
...
```mermaid
flowchart TD
    A["Agent bound with BOTH tools:<br/>standard_booking + vip_launch_booking"] --> B["Normal (non-VIP) user asks:<br/>'Book me a VIP launch seat'"]
    B --> C["Model sees vip_launch_booking is available<br/>and can still choose to call it"]
    C --> D["❌ Live-demonstrated failure:<br/>model calls the VIP tool anyway"]
    D --> E["Correct fix: don't load/bind<br/>vip_launch_booking at all<br/>for non-VIP users"]
    E --> F["Tool structurally unavailable —<br/>model cannot call what it never sees"]
```
...
* CineBot's smart, structured-output **brain still cannot answer real-world questions or take real-world action** — this is what motivates **Tools**.
* A tool is defined with **`@tool`**, deriving its name and description from the function name and docstring by default (both overridable) — genuinely "just a glorified API call" or function, nothing magical.
* Attaching a Pydantic **`args_schema`** provides real, demonstrated validation benefits over plain type hints or a plain JSON schema.
* **`config`** and **`runtime`** are **reserved argument names** — using either causes a real *runtime* error (not a definition-time error), live-reproduced and explained.
* **Binding vs. execution** is reconfirmed yet again: a model can only *request* a tool call — it never executes one.
* **Server-side tools** (built-in model capabilities like web search) run on the provider's own infrastructure, never your machine — distinct from the custom/local tools this session focuses on.
* **Pre-built tools** (like Tavily, explored at the source-code level) are shown to be exactly the same kind of well-documented function you'd write yourself.
* The session's centerpiece, **`ToolRuntime`** — a special, model-invisible parameter (the "mirror" analogy: the model only sees its own reflection, not the world behind it) — gives a tool access to:
  * `state` — short-term memory
  * `context` — immutable per-invocation config
  * `store` — long-term memory (demonstrated live with a working save/recall preference example using `runtime.store.put`/`.get`)
  * `stream_writer` — real-time progress updates
  * `execution_info` — run/thread/attempt IDs, directly tied to the fact that every tool is, underneath, a LangGraph node
* **`return_direct=True`** skips the model's second, paraphrasing call entirely, returning a tool's raw output verbatim — motivated by a real, cited legal case (Air Canada's chatbot being held liable for inventing policy details during paraphrasing).
* **Dynamic tool loading** addresses two real problems:
  * Cost/confusion — demonstrated via real token counts (Haiku loads all tools upfront; smarter models load them only as needed).
  * Genuine security risk — live-demonstrated: a non-VIP user successfully triggering a VIP-only booking tool, proving that trusting the model's own judgment alone is not a reliable safety mechanism.
* The correct fix is **not loading inappropriate tools at all** — a concept explicitly bridging toward **middleware**, deferred to the next session.
...
**Question:** Why can't CineBot's brain alone answer a question about real movie showtimes?
...
**Why Interviewers Ask This:** The core motivation for tools existing at all.

**Possible Follow-up:** "Does structured output fix this problem?"

**Q2.**

**Question:** What does the `@tool` decorator derive by default from a plain function?
...
**Why Interviewers Ask This:** Core, practical tool-definition knowledge.

**Possible Follow-up:** "Why might you want to override the default name?"

**Q3.**

**Question:** Name the two reserved tool argument names in LangChain.

**Answer:** `config` and `runtime`.

**Explanation:** Directly, live-reproduced as a real bug.

**Why Interviewers Ask This:** A genuinely common, practical gotcha.

**Possible Follow-up:** "At what point does using one of these names actually cause an error?"

**Q4.**

**Question:** Does binding a tool to a model cause that tool to be executed when the model is invoked?
...
**Why Interviewers Ask This:** The single most repeated concept in this entire course.

**Possible Follow-up:** "What actually performs the execution?"

**Q5.**

**Question:** Does a web search performed by ChatGPT or Claude run on your own machine?
...
**Why Interviewers Ask This:** A frequently-confused distinction.

**Possible Follow-up:** "What's the difference between this and a custom tool you define yourself?"

**Q6.**

**Question:** What does `ToolRuntime` give a tool access to that the model itself can never see?
...
**Why Interviewers Ask This:** A genuinely advanced, differentiating topic.

**Possible Follow-up:** "Why is this information hidden from the model specifically?"

**Q7.**

**Question:** What are `runtime.store.put()` and `runtime.store.get()` used for?
...
**Why Interviewers Ask This:** A real, practical, production-relevant pattern.

**Possible Follow-up:** "Is InMemoryStore durable across a server restart?"

**Q8.**

**Question:** What does `return_direct=True` do?
...
**Why Interviewers Ask This:** A concrete safety mechanism against a specific, real-world hallucination risk.

**Possible Follow-up:** "What kind of tool output is a good candidate for return_direct?"

**Q9.**

**Question:** Why is loading every possible tool for every user a problem?
...
**Why Interviewers Ask This:** A real, production-relevant design consideration.

**Possible Follow-up:** "What's the correct fix, according to this session?"

**Q10.**

**Question:** What is `InMemoryStore`, and is it suitable for production long-term memory?
...
**Why Interviewers Ask This:** Prevents a common misunderstanding about persistence guarantees.
...
**Question:** Explain, precisely, why using `config` as a tool argument name fails at runtime rather than at definition time.
...
**Why Interviewers Ask This:** Tests precise understanding of *why*, not just *that*, this fails.

**Possible Follow-up:** "What practical debugging implication does this timing have?"

**Q12.**

**Question:** Why does a Pydantic-backed `args_schema` catch a type mismatch (like an integer instead of a string) that a plain JSON schema does not?
...
**Why Interviewers Ask This:** Tests the ability to explain a demonstrated behavioral difference mechanistically.

**Possible Follow-up:** "Would this same distinction apply if you used Python dataclasses instead of a plain JSON schema?"

**Q13.**

**Question:** A learner argues that adding rich field descriptions to a tool's schema wastes tokens unnecessarily. How does the instructor's philosophy directly counter this?
...
**Why Interviewers Ask This:** Tests understanding of a genuine cost-reliability trade-off, not just rote recall.

**Possible Follow-up:** "Under what circumstance might minimizing schema verbosity genuinely be the right call instead?"

**Q14.**

**Question:** Explain the mirror analogy precisely: what does the model see, and what does the tool (via runtime) see?
...
**Why Interviewers Ask This:** Tests whether a learner can explain the mechanism, not just recite the analogy.

**Possible Follow-up:** "Why is it important that the model specifically cannot see this information?"

**Q15.**

**Question:** Using the paid-vs-free ChatGPT/Claude example, explain how `runtime.context` and `runtime.store` together explain why paid AI products often "feel smarter."
...
**Why Interviewers Ask This:** Tests the ability to connect abstract runtime mechanics to a concrete, familiar real-world observation.
...
**Question:** Why does the instructor say every tool is "just a node" in LangGraph, and why does this matter for understanding `runtime.execution_info`?
...
**Why Interviewers Ask This:** Reinforces the "LangChain is built on LangGraph" theme repeated across this course.
...
**Question:** Explain, precisely, what `return_direct=True` prevents, using the Air Canada example.
...
**Why Interviewers Ask This:** Tests whether a learner understands the causal mechanism, not just "return_direct prevents hallucination" as a slogan.

**Possible Follow-up:** "Would setting temperature to 0 have been an equally reliable fix? Why or why not, per this session?"

**Q18.**

**Question:** Why does the instructor argue that "trust the system prompt" is not a sufficient safety mechanism for preventing a non-VIP user from booking a VIP seat?
...
**Why Interviewers Ask This:** A genuinely important lesson about the limits of prompt-based control alone.
...
**Question:** Why does Haiku loading all tools upfront (versus Sonnet/Opus/Fable loading tools only as needed) matter beyond just "using more tokens"?
...
**Why Interviewers Ask This:** Tests whether a learner sees dynamic tool loading as a two-dimensional problem (cost AND correctness), not just a cost-optimization concern.
...
**Question:** What genuinely advanced complication does the instructor raise regarding an agent connecting to an MCP server mid-execution, and why is it "not a straightforward, single-line-of-code problem"?
...
**Why Interviewers Ask This:** Tests whether a learner can recognize the boundary of what this session actually solved versus what remains a real, harder open problem.
...
**Question:** Design a `ToolRuntime`-based solution to the VIP-vs-normal-user problem from Section 12 that does NOT rely on dynamic tool loading (which the session explicitly defers as a middleware topic) — using only `runtime.context` and the tool's own internal logic.
...
**Question:** Critically evaluate: "Since `return_direct=True` skips the model's paraphrasing step, it makes a tool's output completely immune to hallucination." Is this accurate?
...
**Question:** Design a `runtime.store`-based schema for a genuinely production-realistic version of CineBot's preference system, supporting BOTH short-term (per-conversation) and long-term (cross-conversation) preferences for the same user, and explain which `runtime` component you'd use for each.
...
**Question:** Explain precisely why the instructor's live demonstration of the reserved-argument-name bug (Section 4) is a more convincing teaching device than simply stating the rule.
...
**Question:** Synthesize Sections 8 (ToolRuntime) and 12 (dynamic tool loading) to design a coherent explanation of why `ToolRuntime`'s `context` component is described as a bridge toward middleware, the next session's topic.
